  ...
  const InnoLinkPicker = defineComponent({
    ...
    setup(props, { emit }) {
      ...
      function syncFromModel(val) {
        ...
        if (t === "custom") {
          customUrl.value = val.link || val.url || "";
          selectedEntity.value = null;
          return;
        }
        customUrl.value = "";
        // InnoCMS backend contract: { value (id/slug), entity_label, link, entity_image, entity_price }.
        // Accept legacy JS shape (id/name/url/image/...) too for back-compat.
        const id = val.value != null ? val.value : val.id;
        const hasId = id !== undefined && id !== null && id !== "";
        selectedEntity.value = hasId
          ? {
              id: id,
              name: val.entity_label || val.name || "",
              url: val.link || val.url || "",
              image: val.entity_image || val.image || "",
              price_label: val.entity_price || val.price_label || "",
              sku_code: val.entity_sku || val.sku_code || "",
            }
          : null;
      }
        ...
        if (isCustom.value) {
          ...
          if (selectedEntity.value) {
            val = {
              ...
              value: String(selectedEntity.value.id != null ? selectedEntity.value.id : ""),
              entity_label: selectedEntity.value.name || "",
              link: selectedEntity.value.url || "",
              entity_image: selectedEntity.value.image || "",
              entity_price: selectedEntity.value.price_label || "",
            };
          } else {
            val = null;
          }
      ...
      function onTypeChange(val) {
        ...
        if (val !== "custom") {
          ...
          const cleared = { type: val, value: "", entity_label: "", link: "", entity_image: "", entity_price: "" };
