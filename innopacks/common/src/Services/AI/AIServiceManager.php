...
class AIServiceManager
{
     ...
     * @param  string|null  $purpose  Purpose of generation (e.g., content, translation)
    ...
    public function generate(string $prompt, ?string $purpose = null, array $options = []): string
    {
        try {
            ...
            $prompt  = fire_hook_filter('ai.generate_prompt', $prompt, $purpose, $model);
            $options = fire_hook_filter('ai.generate_options', $options, $purpose, $model);
            ...
            try {
                ...
                return fire_hook_filter('ai.generate_result', $result, $prompt, $purpose, $model);
            } catch (\Exception $e) {
    ...
    public function stream(string $prompt, ?string $purpose = null, array $options = []): iterable
    {
        ...
        foreach ($service->stream($prompt, $options) as $chunk) {
            yield fire_hook_filter('ai.generate_result', $chunk, $prompt, $purpose, $model);
        }
    }
