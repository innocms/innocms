...
<div class="card h-min-600">
  <div class="card-body p-4">
    <div class="d-flex flex-wrap align-items-center gap-2 gap-md-3 text-muted small mb-3 pb-3 border-bottom">
      ...
      <span>{{ __('panel/themes.themes_stat_demo') }}: {{ $themes_with_demo_count ?? 0 }}</span>
      <span class="text-secondary">·</span>
      ...
      <div class="themes-wrap">
        <div class="row g-4">
          ...
          <div class="col-6 col-lg-4 col-xxl-3">
            <div class="card themes-item overflow-hidden h-100 border @if(!empty($theme['selected'])) theme-current @endif">
              <div class="theme-image-wrapper position-relative">
                ...
                <div class="theme-overlay">
                  <button type="button"
                    ...
                    {{ __('panel/themes.preview') }}
                  </button>
                </div>
                @if(!empty($theme['selected']))
              ...
              <div class="card-body d-flex flex-column">
                <div class="theme-header mb-3">
                  <h6 class="theme-name mb-2 fw-semibold @if(!empty($theme['selected'])) text-primary @endif d-flex align-items-center gap-2">
...
$(function () {
  ...
  $(document).on('click', '.theme-enable-btn', function (e) {
    ...
    $.ajax({
      ...
      error: function (xhr) {
        var msg = '{{ __("panel/common.updated_success") }}';
  ...
  $(document).on('change', '.theme-switch > input[role="switch"]', function () {
    ...
    axios.put(url, {status: status}, {
      ...
      inno.msg(err.message || '{{ __("panel/common.updated_success") }}');
    }).finally(function () {
      layer.closeAll('loading');
    });
  });
