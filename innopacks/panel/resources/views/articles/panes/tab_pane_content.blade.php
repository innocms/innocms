<div class="tab-pane fade mt-3" id="content-tab-pane" role="tabpanel" aria-labelledby="content-tab" tabindex="0">
  ...
  {{-- 内容 --}}
  <div class="mb-4">
    <div class="col-form-label required"><span class="text-danger">* </span>{{ __('panel/article.content') }}</div>
  ...
  <div class="mb-3 border-top pt-3">
    <div class="col-form-label">{{ __('panel/article.image') }}</div>
    <ul class="nav nav-tabs" id="locales-image-tab" role="tablist">
      @foreach (locales() as $locale)
        <li class="nav-item" role="presentation">
          <button class="nav-link d-flex {{ $loop->first ? 'active' : '' }}" id="locale-{{ $locale->code }}-image-tab"
            data-bs-toggle="tab" data-bs-target="#locale-{{ $locale->code }}-image-pane" type="button"
            role="tab" aria-controls="locale-{{ $locale->code }}-image-pane"
            aria-selected="{{ $loop->first ? 'true' : 'false' }}">
            <div class="wh-20 me-2">
              <img src="{{ image_origin($locale->image) }}" class="img-fluid">
            </div>
    ...
    <div class="tab-content mt-3" id="locales-image-tabContent">
      @foreach (locales() as $locale)
        @php($localeCode = $locale->code)
        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
          id="locale-{{ $localeCode }}-image-pane" role="tabpanel"
          aria-labelledby="locale-{{ $localeCode }}-image-tab" tabindex="0">
          <div class="d-flex justify-content-center">
            <x-panel-form-image title="" name="translations[{{ $localeCode }}][image]"
