...
<head>
  ...
  <title>CLI Login — {{ config('app.name', 'InnoCMS') }}</title>
    ...
    <div class="login-wrap">
      <div class="card login-content">
        <div class="card-header">
          ...
          <p class="text-center text-muted mt-2 mb-0">Authorization request from CLI</p>
        </div>

        <div class="card-body">
          <form action="{{ panel_route('cli-login.store') }}" method="post">
            @csrf

            <div class="form-floating mb-4">
              <input type="text" name="email" class="form-control" id="email-input" value="{{ old('email') }}" placeholder="{{ __('panel/login.email') }}">
            ...
            <div class="form-floating mb-5">
              <input type="password" name="password" class="form-control" id="password-input" placeholder="{{ __('panel/login.password') }}">
