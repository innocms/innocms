...
namespace InnoCMS\Aicore\Http\Controllers\Panel;

use Exception;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use InnoCMS\Aicore\Agents\ContentAgent;
use InnoCMS\Aicore\Services\AIServiceManager;
...
class ContentAIController extends BaseController
{
    ...
    private function buildEntityContext(string $type, int $id): array
    {
        ...
        try {
            if ($type === 'product') {
                $product = Product::query()->with(['translation', 'categories.translation'])->find($id);
                if (! $product) {
                    return [];
                }

                $title    = trim((string) ($product->translation->name ?? ''));
                $category = $product->categories
                    ->map(fn ($c) => trim((string) ($c->translation->name ?? '')))
                    ->filter()
                    ->implode(', ');
                ...
                return array_filter([
                    'title'    => $title,
                    'category' => $category,
                    'spu_code' => trim((string) ($product->spu_code ?? '')),
                ]);
            }
