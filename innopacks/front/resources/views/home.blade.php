...
@section('content')
  ...
  @if(($latestNews ?? null) && $latestNews->isNotEmpty())
    <div class="home-section">
      <div class="container">
        <div class="module-title" data-aos="fade-up">{{ __('front::common.news_title') }}</div>
        <div class="module-sub-title" data-aos="fade-up">{{ __('front::common.news_sub') }}</div>
        <div class="row g-4">
          @foreach($latestNews as $article)
            <div class="col-12 col-md-6 col-lg-4" data-aos="fade-up" data-aos-duration="{{ 300 + $loop->index * 200 }}">
              <div class="card h-100 shadow-sm rounded-3 overflow-hidden news-card">
                <a href="{{ $article->url }}" class="news-card-img-wrap">
                  <img src="{{ image_resize($article->translation->image ?? '', 800, 450) }}" class="card-img-top" alt="{{ $article->translation->title ?? '' }}">
                  <span class="news-badge">{{ __('front::common.news') }}</span>
                </a>
                <div class="card-body d-flex flex-column">
                  <div class="news-card-cat mb-2">
                    @if($article->catalog && $article->catalog->translation)<i class="bi bi-folder2"></i> {{ $article->catalog->translation->title }}@endif
                  </div>
                  <h3 class="h5 fw-bold mb-2"><a href="{{ $article->url }}" class="text-reset text-decoration-none">{{ $article->translation->title ?? '' }}</a></h3>
                  <p class="text-muted flex-grow-1 mb-0">{{ \Illuminate\Support\Str::limit($article->translation->summary ?? '', 110) }}</p>
                </div>
                <div class="card-footer bg-transparent border-0 pt-0">
                  <a href="{{ $article->url }}" class="news-card-link">{{ __('front::common.read_more') }} <i class="bi bi-arrow-right"></i></a>
                </div>
              </div>
            </div>
          @endforeach
